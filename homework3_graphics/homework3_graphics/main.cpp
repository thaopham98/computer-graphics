...
void textStar(float x, float y, int size, char* msg){
    glRasterPos3f(x*0.38, y*0.2, 0.0);
    writeBitmapString(GLUT_BITMAP_8_BY_13,/*"Star is Selected. Press \"SPACE\" to Spint"*/ msg);
}
...
// -------------------- */ /* MARK: Keyboard input processing routine */ /* -------------------- */
        ...
        case ' ':
            if (triangle || diamond || star || cross)
            {
                ...
                if (pauseMotion) // true == stop spinning
                {
                    ...
                    {
                        std::cout << "Number of Shape: " << shapeIndex << "\n";
                        std::cout << "Pointed Shape: " << pointedShape << "\n";
                        std::cout << "You win!\n";
                    }
                    else
                    {
...
//                else /* --- not true == false == spinning --- */
//                {
//                    pauseMotion = true; /* --- stop the arrow --- */
//                }
            }
            break;
        ...
        case 'r':
            ...
            transforms = true;
            totalDegreeR = 0+ 90;
            triangle = star = diamond = cross = false;
            if (pauseMotion)
            {
                std::cout << "The shapes are spinning.\n" << "TotalDegreeR: "<< totalDegreeR << "\n The rotR: " << rotR << '\n';
                pauseMotion = false; // spin the shapes
                transforms = true;
            }
            else
            {
...
//            std::cout <<"\nrotZ in the timerFunc(): " << rotZ << '\n';
    ...
    else if(transforms == true)
    {
        ...
        {
            rotR = (rotR + 5);
            std::cout <<"\nrotR in the timerFunc(): " << rotR << '\n';
        }
        else if (rotZ == totalDegree)
            pauseMotion = true; // stop spinning
    }
    
//    if (rotZ > 360) rotZ = 0; // setting back to zero == won't stop spinning
    glutPostRedisplay();
...
//        glTranslatef(-250.0 , -250.0 , 0.0); // maybe/not???
//    }
    
    /* ---------- BLACK FRAME FOR THE SHAPES ---------- */
    if (triangle){
        glColor3f(0.0,0.0,0.0);
        draw_Triangle(obj_x, obj_y, size);
        textTriangle(obj_x, obj_y, size);
    }
    else if (cross){ //outline
        glColor3f(0.0,0.0,0.0);
        draw_Cross(obj_x, obj_y, size);  //outline
        textCross(obj_x, obj_y, size);
    }
    else if (diamond){ //outline
        glColor3f(0.0,0.0,0.0);
        draw_Diamond(obj_x, obj_y, size);
        textDiamond(obj_x, obj_y, size); //outline
    }
    else if (star){
        glColor3f(0.0, 0.0, 0.0);
        draw_Star(obj_x, obj_y, size);
        textStar(obj_x,obj_y,size, "Star is selected. Please press \"SPACE\" to spin.");
    }
...
//    glTranslated(-size*2.62, size*0.025, 0.0);
    if (transforms == true){
        /* --- RED CROSS --- */
        glColor3f(1.0, 0.0, 0.0);
        glRotatef(rotR, 0 , 0 , 1);
        glTranslated(120.0, 0.0, 0.0); // where it'll be after translation based on the black do location
        glRotatef(-rotR, 0 , 0 , 1);
        drawCross(119.5, 250.0, size); // the black dot location
        glLoadIdentity();
        ...
        glRotatef(-rotR, 0 , 0 , 1);
        drawDiamond(250.0, 375.0, size); // the black dot location
        glLoadIdentity();
